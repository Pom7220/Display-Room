...
- hypothesis: An HTTPS content filter or captive portal on the Lobby VLAN intercepts Cloudflare traffic. The Office VLAN was configured in July 2026; the Lobby VLAN was not. Static IP change did not fix it — the policy is not IP-range based. Root cause requires network engineer to compare Lobby vs Office VLAN outbound HTTPS policies for Cloudflare IPs.
- what NOT to do: Do not run `am start` on our APK on Doppio while Meet in Touch is in operation — the 10-min health check alarm will keep relaunching our APK and disrupting Meet in Touch.
- fix applied: None — Doppio returned to Meet in Touch operation on 2026-09-10.
- next step: Ask network engineer what outbound HTTPS policy applies to Lobby VLAN (10.0.54.81 range) for Cloudflare IPs (172.67.213.200 / 104.x.x.x) that differs from Office VLAN.
