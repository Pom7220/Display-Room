...
- status: MOOT as of 2026-09-22 — both tasks deleted, but the warning still applies to any new scheduled task
- 2026-09-22 update: `ris-health-morning` and `ris-health-evening` were disabled on 2026-09-17 and DELETED on 2026-09-22 at the user's request — they were not contributing enough to justify the runs. Their `SKILL.md` files remain on disk at `C:\Users\k_rat\.claude\scheduled-tasks\ris-health-*\` and the master definition `.claude/agents/ris-health-agent.md` is still git-tracked, so the agent can be revived if wanted. Do not re-create these tasks without asking.
- **carry-forward warning**: the drift described below was a property of the SCHEDULER, not of those two tasks. The replacement task `ris-standby-capture` (09:06 Mon–Fri) runs on the same scheduler and may fire hours off its slot. That task tolerates drift — it only needs to pull before the next 06:00 reboot and re-arm before midnight — but any future task with tight timing must not assume its cron slot is honoured. Always check the actual clock at run time rather than trusting the task's own label.
- seenCount: 3
...
- **Two different `approval` flags — do not confuse them.** `RIS_ROOMS.approval:true` (ris-shared.js, Macchiato only) drives the DASHBOARD, e.g. excluding it from default room selection. The tablet's `.appr-banner` is driven by the tablet's OWN local `cfg.approval` from its settings screen (`var na = cfg.booking && cfg.approval` in `openModal`). **As of 2026-09-22 that toggle is off everywhere, so no tablet shows the approval banner** — verified by photo on Macchiato, which renders the M365 `.book-note` exactly like every other room. A 2026-09-22 session initially assumed Macchiato's sheet was the fleet's tallest because of RIS_ROOMS; it is not.
- if `Require owner approval` is ever enabled on a tablet, `.appr-banner` replaces `.book-note`. Post-v3.10.221 compression the two are within 2px, so that sheet ≈ 831px, ~27px under the cap — still fine, but re-measure if the banner text changes.
- verified by photo 2026-09-22 on Macchiato with the keyboard raised: QR, both fields, start time, duration chips, note, Cancel and Confirm booking all visible, no scrolling.
